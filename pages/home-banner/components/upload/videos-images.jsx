...
import { setDeleteMedia } from "../../../../store/home/slice";

import styles from "../../../../components/uploads/upload-media/styles.module.scss";
              ...
              images?.map((item, index) => (
                <li key={index}>
                  ...
                  item?.[`file_type_${lang}`] === "image" ? (
                    <img
                      alt={`Media for ${lang}`}
                      src={`${BASE_URL}${item?.[`path_${lang}`]}`}
                    />
                  ) : item?.[`file_type_${lang}`] === "video" ? (
                    <video width="150" height="100" controls>
                      <source src={`${BASE_URL}${item?.[`path_${lang}`]}`} />
                    </video>
                  ) : (
                    //? which case this code will be executed ?
                    <img src={`${BASE_URL}${item}`} alt={`Media for ${lang}`} />
                  )}
                  <ClearIcon
